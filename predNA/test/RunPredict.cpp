...
int runRefinementsub(NuPairMoveSetLibrary* moveLib, EdgeInformationLib* eiLib, RnaEnergyTable* et, const string& inputFile, const string& outFile, int randSeed, double kStep, bool printDetail){
	...
	int corePosssub_size = 4;
	int corePossub[corePosssub_size] = {192, 205, 101, 114}; 
	int corePos = 321;
	int count_while = 0;
	...
	while (true)
	{
		// if(count_while >= corePosssub_size){
		// 	break;
		// }
		// corePos = corePossub[count_while];
		// count_while++;
		...
		cout << "csMoveLists.size: " << csMoveLists.size() << endl;
		cout << "csMoveLists[i].size: ";
		for(size_t i = 0; i < csMoveLists.size(); i++){
			cout << csMoveLists[i].size() << " ";
		}
		cout << endl;
		...
		cout << "outsubGraphPosList: ";
		for (size_t i = 0; i < outsubGraphPosList.size(); i++) {
			cout << outsubGraphPosList[i] << " " ;
		}
		cout << endl;
        ...
        vector<int> ().swap(outsubGraphPosList);  // 清空 outsubGraphPosList
