...
BRConnection::BRConnection(FragmentLibrary* fragLib, const string& connectType, BRNode* fatherNode, BRNode* childNode, int seqLen){
	...
	this->fixed = false;
	this->mvRotType = 0;
	this->cm = childNode->baseConf->cs1 - fatherNode->baseConf->cs1;
	this->cmTmp = this->cm;
	...
	this->childRot = childNode->riboseConf->rot;
	this->childRotTmp = childNode->riboseConf->rot;
	...
	else if(connectType == "revWcNb")
		this->f2Lib = fragLib->revWcNb[fatherNode->baseType*4 + childNode->baseType];
	...
	*/
		...
		cout << "undefined connection type" << endl;
		exit(0);
	}
