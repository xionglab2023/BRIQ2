...
void MCRun::simpleMC(const string& outPDB, bool printTraj){
	...
	cout << "tn: " << tn << endl;
	cout << "rfn: " << rfn << endl;
	cout << "freeNodeNum: " << freeNodeNum << endl;
...
void MCRun::debug(){
	...
	for(int i=0;i<10000;i++){
		...
		CsMove cm2 = ft->nodes[4]->baseConfTmp->cs1 - ft->nodes[3]->baseConfTmp->cs1;
	...
	*/
	...
	for(int i=0;i<10000;i++) {
		...
		if(i%2==0){
			cout << "accept connection node: "  << endl;
			ft->acceptRiboseRotamerTmp(node, false);
			totE += mutE.second;
		}
		else {
			cout << "reject connection node: " << endl;
			ft->clearRiboseRotamerTmp(node, false);
		}
