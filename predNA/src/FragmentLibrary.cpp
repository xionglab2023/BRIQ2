...
F2Fragment::F2Fragment(const string& line,int typeA, int typeB, RotamerLib* rotLib){
...
F3Fragment::F3Fragment(const string& line,int typeA, int typeB, int typeC, RotamerLib* rotLib){
	...
	this->rotA = rotLib->riboseRotLib->rotLib[typeA][a];
	this->rotB = rotLib->riboseRotLib->rotLib[typeB][b];
	this->rotC = rotLib->riboseRotLib->rotLib[typeC][c];
	this->plA = NULL;
	this->plB = NULL;
}
...
FragmentLibrary::FragmentLibrary(RotamerLib* rotLib) {
	...
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			...
			revWcNb.push_back(new F2FragmentLib("revWcNb", i, j, rotLib));
