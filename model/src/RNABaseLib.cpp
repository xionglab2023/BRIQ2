...
vector<XYZ> RNABaseLib::getBaseSidechainCoords(int baseType){
    vector<XYZ> tList;
    if(baseType == 0){
...
vector<XYZ> RNABaseLib::getPolarAtomCoords(int baseType){
    vector<XYZ> tList;
    if(baseType == 0){
            XYZ c = XYZ(3.577,  -0.770,   0.000); //A-N7
            XYZ f = XYZ(5.904,   1.230,   0.000); //A-N6
...
RNABase* RNABaseLib::getBase(const string& baseID, const string& chainID, int baseType, LocalFrame& cs, RiboseRotamer* rot){
    ...
    for(int i=0;i<rot->atomNum;i++){
    	base->addAtom(new Atom(names[i], local2global(cs, rot->localCoords[i])));
    }

	return base;
}

RNABase* RNABaseLib::getBase(const string& baseID, const string& chainID, int baseType, LocalFrame& cs, RiboseRotamer* rot, PhosphateRotamer* pRot){
	RNABase* base = getBase(baseID, chainID, baseType, cs);
    vector<string> names;
	names.push_back("C1'");
    names.push_back("C2'");
    ...
    PhosphateConformer* pho = new PhosphateConformer(pRot, cs2);

	base->addAtom(new Atom("P", pho->coords[0]));
	base->addAtom(new Atom("O5'", pho->coords[1]));
	base->addAtom(new Atom("OP1", pho->coords[2]));
	base->addAtom(new Atom("OP2", pho->coords[3]));
	delete pho;
	return base;
}
