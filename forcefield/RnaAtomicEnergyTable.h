...
#ifndef FORCEFIELD_RNAATOMICENERGYTABLE_H_
#define FORCEFIELD_RNAATOMICENERGYTABLE_H_
...
public:
	...
	RnaAtomicEnergyTable(XPara* para);
	...
	double getPhoPhoHbondEnergy(XYZ* phoA, XYZ* phoB){
		...
		XYZ a = phoA[0];
		XYZ d = phoB[0];
		if(squareDistance(a,d) > 64) return 0;
		...
		if(minLen > 6.0)
			...
			rep = para->phoRep*0.25*minLen + 0.833333*para->phoRep;
		else
			rep = para->phoRep/minLen - para->phoRep*0.166667;

		return hbondEne + rep;
	}

	virtual ~RnaAtomicEnergyTable();
};

} /* namespace NSPforcefield */

#endif /* FORCEFIELD_RNAATOMICENERGYTABLE_H_ */
